#!/bin/bash
#SBATCH --job-name=lmvf_gen_solutions
...
# Run generation script per task, assigning specific GPUs via CUDA_VISIBLE_DEVICES
srun bash -c 'echo "Running generate_solutions.py with SLURM_NTASKS=$SLURM_NTASKS and SLURM_PROCID=$SLURM_PROCID on GPUs $SLURM_LOCALID"; \
              ...
              python scripts/py/generate_solutions.py --config-path '"$CONFIG_FILE"' --num-tasks=$SLURM_NTASKS --task-id=$SLURM_PROCID'
